...
如果路由器上没有默认配置，则有几种选择，这里用一种适合需求的方法。
...
由于 MAC 连接不是很稳定，需要做的第一件事是设置路由器以便 IP 连接可用：
...
现在世界上任何人都可以访问路由器，因此现在是保护它免受入侵者和基本攻击的最佳时机

## 用户密码访问

MikroTik 路由器需要配置密码，建议使用密码生成器工具来创建安全且不重复的密码。 安全密码的意思是：
...
强烈建议使用第二种方法或 Winbox 界面为你的路由器应用新密码，只是为了防止其他未经授权的访问。
...
首先，要添加一个 NAT 规则来将 HTTP 重定向到代理。 用运行在端口 8080 上的 RouterOS 内置代理服务器。
