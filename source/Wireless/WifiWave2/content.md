...
在继续之前，先熟悉一下对于理解WifiWave2的操作很重要的术语。这些术语将贯穿全文。
...
虽然无线电信息提供了有关支持通道宽度的信息，但也可以从产品页面推断出此信息，为此您需要检查以下参数:链数，最大数据速率。一旦知道了这些参数，就需要查看调制和编码方案(MCS)表，例如: [https://mcsindex.com/](https://mcsindex.com/)。

以hAP ax <sup>2</sup> 为例，可以看到MCS表中的链数为2，最大数据速率为1200 - 1201。在MCS表中，需要找到2个空间流链的入口，以及各自的数据速率，在这种情况下，80MHz是支持的最大信道宽度。
