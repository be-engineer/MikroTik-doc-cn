...
在ROS v7中，此Regex模式将匹配AS路径中间的任何地方的ASN 1234，在ROS v6中，相同的模式将匹配包含ASN至少由6个字符组成并包含字符串“1234”的任何AS路径。显然，如果直接将Regex模式从一个实现复制粘贴到另一个实现，将会导致意想不到的危险结果。在ROS v6中，一个等效的模式看起来像这样:".\_1234\_."。
