...
所有存储在路由器本地存储器中的信息都可以从 `/log` 菜单打印出来。每条都包含事件发生的时间和日期、消息所属的主题和消息本身。
...
| **syslog-severity** (_alert, auto, critical, debug, emergency, error, info, notice, warning_; Default: **auto**)                                                                                                                                 | RFC 3164中定义的严重程度： <br>- emergency：系统无法使用<br>- alert：必须立即采取行动<br>- critical：危急情况<br>- error：错误状况<br>- warning：警告条件<br>- notice：正常但重要的状况<br>- info：信息性消息<br>- debug: 调试级信息 |
...
每条日志都有描述日志信息来源的主题。可以有一个以上的主题分配给日志信息。例如，OSPF调试日志有四个不同的主题：route、OSPF、debug和raw。
...
结果将记录到 **log.0.txt** 和 **log.1.txt** 文件中。

可以指定 _disk-lines-per-file_ 来指定文件的最大行数。**\<file\>.0.txt** 是活动文件，会添加新的日志，一旦大小达到最大值，就变为 **\<file\>.1.txt**，新文件 **\<file\>.0.txt** 将被创建。

可以在文件名前指定目录名来登录U盘或 _MicroSD/CF_ （在Routerboards）。例如，如果你在 _/files_ 下的 **usb1** 目录下访问u盘，用以下命令。
