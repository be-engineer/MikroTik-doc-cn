...
Traceroute结果显示产生ICMP超时报文上的MPLS标签。上面的意思是:当R3接收到一个MPLS TTL为1的数据包时，它的标签为18。这场比赛由R3为111.111.111.4通告标签。以同样的方式，R2在下一个traceroute迭代中观察到数据包上的标签17 - R3将标签17切换到标签17，如上所述。R1收到没有标签的数据包- R2像上面解释的那样做倒数第二跳弹出。
